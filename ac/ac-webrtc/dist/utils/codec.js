...
exports.preferOpus = void 0;

// Set Opus as the default audio codec if it's present.
var preferOpus = function preferOpus(sdp) {
  var sdpLines = sdp.split('\r\n');
  var mLineIndex; // Search for m line.

  sdpLines.forEach(function (sdpLine, index) {
  ...
  sdpLines.forEach(function (sdpLine) {
    if (sdpLine.search('opus/48000') !== -1) {
      var opusPayload = extractSdp(sdpLine, /:(\d+) opus\/48000/i);
...
var removeCN = function removeCN(sdpLines, mLineIndex) {
  var mLineElements = sdpLines[mLineIndex].split(' '); // Scan from end for the convenience of removing an item.

...
class DecisionPanel extends Component {
  ...
  handleSubmit = e => {
    ...
    if(choosen){
      goodJustif = this.allDef.reduce((acc, x, i) => acc && (x.correct === this.state.valueState[i]), true);
    }else{
      const noTCheckedNoFChecked = this.allDef.reduce((acc, x, i) => x.correct ? [acc[0] && !this.state.valueState[i], acc[1]] : [acc[0], acc[1] && !this.state.valueState[i]], [true, true]);
      ...
      <div style={this.props.style}>
        ...
        <div className="btn-group" role="group" aria-label="...">
          <button
            ...
            tabIndex='-1'
            onClick={() => this.setState({selected: !this.state.selected})}>{this.state.selected ? 'True': ''}</button>
          <button
            className="btn btn-default"
            style={{
            ...
            tabIndex='-1'
            onClick={() => this.setState({selected: !this.state.selected})}>{!this.state.selected ? 'False': ''}</button>
        </div>
          <form

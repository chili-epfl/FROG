...
var updateOffset = function() {
  ...
  HTTP.get(syncUrl, function(err, response) {
    ...
    if (err) {
      ...
      if (++attempts <= maxAttempts) Meteor.setTimeout(TimeSync.resync, 1000);
      else log('Max number of time sync attempts reached. Giving up.');
      return;
    }
...
Meteor.setInterval(function() {
  ...
  var discrepancy = SyncInternals.getDiscrepancy(
    lastClientTime,
    currentClientTime,
    defaultInterval
  );

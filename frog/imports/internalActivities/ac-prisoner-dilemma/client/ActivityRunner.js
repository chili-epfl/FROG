...
import { type ActivityRunnerT, type ActivityRunnerPropsT } from '/imports/frog-utils';
...
class PrisonerDilemmaController extends React.Component<StyledPropsT, any> {
  ...
  render() {
    ...
    const disableButtons =
      ...
      this.props.userInfo.id in
        this.props.data.rounds[(currentRound - 1).toString()];
    ...
    if (this.props.data.phase === 0 && keys.length === 2) {
      const left = this.props.data.rounds[(currentRound - 1).toString()][
        keys[0]
      ];
      const right = this.props.data.rounds[(currentRound - 1).toString()][
        keys[1]
      ];
      ...
      if (this.props.activityData.config.rounds !== currentRound) {
        this.props.dataFn.objInsert({}, ['rounds', currentRound.toString()]);
      } else {

...
const Runner = ({ activity }) => {
  ...
  const onCompletion = (completionData) => {
    addProduct(activity._id, activity.activityType, Meteor.userId(), completionData)
  }
  const inputRaw = Results.findOne({ activityId: activity._id, type: 'product' })
...
const TimedRunner = ({ activity, timeNow }) => {
  ...
  if (duration === 0) { return (<Runner activity={activity} />) }
  if (seconds < 0) { return (<h1>Time-out for this activity</h1>) }
  return (
    <div>

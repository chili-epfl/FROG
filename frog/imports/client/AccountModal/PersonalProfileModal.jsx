...
export const PersonalProfileModal = () => {
    ...
    const onChangeDisplayName = newDisplayName => {
      ...
      const onChangePassword  = (oldPassword, newPassword) => {
        Accounts.changePassword(oldPassword, newPassword, err => {
          if (err) { 
            if (err.error === 403){
              showToast('Your old password is incorrect. Please try again', 'error'); 
            }
            else 
    ...
    const ComponentToRender = () =>  {
        switch(form){
            case 'DisplayName': return (<ChangeDisplayNameForm onChangeDisplayName = {onChangeDisplayName} />)
            case 'Password': return (<ChangePasswordForm onChangePassword = {onChangePassword}/>)
            default : return (<PersonalProfileForm openPasswordForm = {openPasswordForm} openDisplayNameForm = {openDisplayNameForm} /> )
        }
    }
